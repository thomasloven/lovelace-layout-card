...
The grid layout's `style` option accepts both `margin` and `padding` parameters that set the respective CSS attribute for the grid layout's container. These parameters accepts values in the CSS [`margin`](https://developer.mozilla.org/en-US/docs/Web/CSS/margin) and [`padding`](https://developer.mozilla.org/en-US/docs/Web/CSS/padding) shorthand formats.
